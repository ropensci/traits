...
#' @examples \donttest{
#' # A single species
#' out <- ncbi_search_(taxa="Umbra limi", seqrange = "1:2000")
#' # Get the same species information using a taxonomy id
#' out <- ncbi_search_(id = "75935", seqrange = "1:2000")
#' # If the taxon name is unique, using the taxon name and id are equivalent
#' all(ncbi_search_(id = "75935") ==  ncbi_search_(taxa="Umbra limi"))
...
#' #  same name
#' nrow(ncbi_search_(id = "266948")) ==  nrow(ncbi_search_(taxa="Satyrium"))
...
#' out <- ncbi_search_(taxa="Sequoia wellingtonia", seqrange="1:2000", getrelated=TRUE)
#'
#' # Many species, can run in parallel or not using plyr
#' species <- c("Salvelinus alpinus","Ictalurus nebulosus","Carassius auratus")
#' out2 <- ncbi_search_(taxa=species, seqrange = "1:2000")
...
#' ncbi_search_(taxa = "Olpidiopsidales", limit = 5, getrelated = TRUE,
#'             entrez_query = "18S[title] AND 28S[title]")
#' }
#' @export
ncbi_search_ <- function(taxa = NULL, id = NULL, seqrange="1:3000", getrelated=FALSE, limit = 500,
